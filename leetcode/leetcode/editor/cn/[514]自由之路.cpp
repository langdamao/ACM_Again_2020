...
// 
// Related Topics 深度优先搜索 广度优先搜索 字符串 动态规划 👍 239 👎 0
...
public:
    int findRotateSteps(string ring, string key) {
       int lr = ring.length();
       int lk = key.length();
       vector<vector<int>> dp(lk+1,vector<int>(lr,INT_MAX));
       for (int i=0;i<lr;i++) dp[0][i] = min(i,lr-i);
       for (int i=1;i<=lk;i++){
           for (int j=0;j<lr;j++){
               if (ring[j]==key[i-1]) {
                   dp[i][j] = min(dp[i][j],dp[i-1][j]+1);
                   for (int k=1;k<lr;k++) {
                       dp[i][(j+k)%lr] = min(dp[i][(j+k)%lr],dp[i][j]+k);
                       dp[i][(j-k+lr)%lr] = min(dp[i][(j-k+lr)%lr],dp[i][j]+k);
                   }
               }
           }
       }
